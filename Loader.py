...
class Loader1396(Loader):

    def load_and_split(self):
        self.all_texts = [
                """.درروستاییبا هزار نفر جمعیت،دویستنفر مبتال به نوعی بیماری شدهاند.با انجام یکآزمایش
میتوان بیماری را تشخیص داد ولی نتیجهخطا دارد،به این معنی که فردبیمار رابه احتمال5
درصدسالم وفردسالم رابه احتمال5درصدبیماراعالممیکند.فردی را به تصادف از مردم این
روستاانتخاس کرده و از او آزمایش میگیریم.چند درصد احتمال داردآن فردبیماراعالمشود؟""",
        """.کارخانهایمحصوالتش را در بستههایی مکعبی بستهبندی میکند و در انبار هر212بسته را به
شکل مکعبی8 8 8به هم متصل میکنند.در شرایطی که دمای هوا از حدی بیشتر شود
الزم است با جدا کردن این مجموعة212تایی با صفحات افقی و عمودی، هر بسته دستکم یک
وجه داشته باشد کهبا وجوه بستههای دیگر در تماس نباشد.کمترین تعداد صفحه برای رسیدن به
چنین وضعیتی چند تا است؟""",
                """.چندعدد هشترقمی در مبنای چهار با ارقام1،2و3وجود دارد که بر3بخشپذیر باشد؟""",
                """.پنجنقطهدر صفحه داریم که هیچ سه تایی همخط نیستند.همة پارهخطهایی که رئوسشان این
نقاط است را رسم کردهایم.حداقل چند نقطة جدید در محل تقاطع پارهخطها به وجود میآید؟
1)صفر2)11)24)15)5
""",
""".دور گود یک زورخانه12۱صندلی، دایرهوار،چیده شده است.تماشاچیاولی که وارد زورخانه
میشود روی صندلی شماره یک مینشیند.از آنجا کهسرماخوردگی در شهر شایع شده،هر فردی
که وارد میشود روی صندلیای مینشیند که فاصلهاش با نزدیکترین صندلی اشغالشده بیشترین
مقدار ممکن باشدواگر چند صندلی با این خاصیت وجود داشتآنکهشمارهاش کمتر استرا
انتخاس میکند.نفر3۳م روی کدام صندلی مینشیند؟""",
""".عددی طبیعیn،کوچکترین عددیاستکه باقیماندةn3 45بر10۰0برابر1۰است.
باقیماندةn17 18بر1۳20چند است؟""",
"""3.مساحتمحصور بین نمودار تابع[ ]y x x2
10 1در بازة[ , ]0 1و محور طولها چهقدر
است؟(منظور از[ ]t،جزء صحیآ عددtاست)""",
""".شمارة اصلیپالکیک خودرو متشکل از یک عدد دو رقمی در سمت چپ، یکی از حروف الفبا در
وسط ویک عدد سه رقمی در سمت راست استواز رقم صفر در این دو عدد استفاده نمیشود.بههر
پالک،ضرسدو عدد مورد اشارهرا نسبت میدهیم.ًمثالبه پالکزیرعدد12 365 4380را
نسبت میدهیم.
میانگیناعدادنسبتدادهشدهبهتمامپالکهای ممکن،چنداست؟""",
""".قسمتی ازپارکجنگلیلتیانکه بیندوجادهواقعشده،آتشگرفتهاست.آتش با سرعت11
کیلومتربر ساعت گسترش مییابد.اگر فاصلةنقطةشروع آتشسوزیتا هر دوجاده11کیلومتر
باشد،پس از2ساعت مساحت ناحیةسوختهشده چند کیلومترمربع است؟(جادههارا خطهای
طوالنی،راستو موازی با همبگیرید وفرضکنیدآتش از جاده عبور نمیکند).
1)200 32)100 31)400 3 400 34)200 3 400 35)400 3""",
"""11.مثلثABCرا در نظر بگیرید کهAB AC 15وBC 18.نقاطDوEرا
بهترتیب رویCAوCBطوری بگیرید کهCD 5وCE 3.نقطةFرا روی نیمخطDE
طوری در نظر بگیرید کهEF 8.نقطةMرا وسطABوNرا نقطة برخوردFM
باBCبگیرید.طولCNچهقدر است؟""",
"""11.طبقضوابط، ساختمانهای یک کوچهنبایدبیش از9طبقهداشتهباشند.بهعالوه اگر تعداد طبقات
دو ساختمان، با هر چهقدر فاصله، برابر باشد، حتما ً باید ساختمانیبا تعداد طبقات بیشتراز آن دو
بینشان وجود داشته باشد.حداکثر چند ساختماندر یک سمت کوچهمیتوان ساخت؟""",
"""12.چند سه تایی( , , )a b cً از اعداد طبیعیاکیداکمتر از21داریمکهa b cبرa،bوc
بخشپذیرباشد؟""",
"""11.فردیمیخواهد کنار دیوار باغش، ناحیهای را برای نگهداری از مرغ و خروسهایش بسازد.او سه
نرده به طول ده متر دارد.با استفاده از دیوار باغ، که مستقیم و طوالنی است و این سه نرده، حداکثر
مساحتی را که میتواند محصور کند چند مترمربع است؟
1)1002)50 25 33)50 50 24)75 32)111""",
"""14.برایچند مقدار صحیآk،دستگاه معادالت زیرجوابیغیرازa b c 0در اعداد
حقیقیدارد؟( )
( )
( )
b kc a b
...
2 2
2 2
1)02)13)24)32)بینهایت""",
"""15.فرضکنید, , ,a a a1 2 20هماناعداد1تا20باشند که به ترتیبی دلخواه نوشته شدهاند.
عبارتa a a a1 2 3 20چندمقدارمختلفمیتواند باشد؟""",
"""۰.زیرمجموعهایاز اعداد حقیقیدارای این خاصیت استکه برای هردو عضو متمایز آن مثلx
وy، داریم( )x y xy2
1 1.این مجموعه حداکثر چند عضو دارد؟
1)12)23)34)42)بینهای""",
"""13.دوعددطبیعیmوnکمتر از200هستندو[ , ] ( )m n m n 2.حداکثر مقدارm n
چهقدر است؟([ , ]m nکوچکترینمضرسمشترکmوnاست""",
"""18.سه میله با طولهای13۳۰،143۳و201۱میلیمترروی زمیناز یکسربههملوال شدهاند.
کمترین مقدار ممکن برای شعاع دایرهای که از سهسردیگر میلههامیگذردچند میلیمتراست؟""",
"""1۳.فرض کنیدx y zاعدادی حقیقیومثبتباشندکهx y z xy yz zx2 2 2 2 2 2
حداقل مقدارx
zکدام است؟
1)12)23)34)22)4""",
"""1.درچهارضلعیمحدس ومحاطیABCDداریمB 0
110.محل برخوردADوBCراE
ومحل برخوردABوCDراFمینامیم.اگر عمودهایEبرABوFبرBCیکدیگر
را روی دایرةمحیطی چهارضلعی در نقطةPقطع کنند.اندازةزاویةPDFچند درجهاست؟""",
"""1.نقطةPدرونیارویمثلث متساویاالضالعABCبه ضلعیکقرار دارد.قرینةPنسبتبهAB
راK،قرینةKنسبت بهBCراMو قرینةMنسبت بهACراNمینامیم.
حداکثرطولNPکدام گزینهاست؟
1)2 32)31)3
24)35)1""",
"""22.هشتعددکاشیمشابه1 2داریم که هر کدامازیکمربعقرمز و یکمربعآبیتشکیل شده
است.به چند حالت میتوان یکناحیة4 4را با اینکاشیهاپوشاند طوری که هر سطر و هر
ستونآندومربعقرمز و دومربعآبی داشته باشد؟""",
"""21.پروانهپرگاری خرید و پس از باز کردن بستة آنمتوجه شد طول
میلةسوزندار10سانتیمتر و طول میلة مدادی1۰سانتیمتر است!
فرض کنیدبرای اینکه بتوان با آن پرگار دایره کشید، باید زاویةمداد
با کاغذ دستکم30درجهباشد ولی سوزن میتواند هر زاویهای با
صفحة کاغذ داشته باشد.تفاضلشعاعبزرگتریندایرهوشعاع
کوچکترین دایرهای که میتوان با این پرگار کشیدبر حسب
سانتیمتر به کدام گزینه نزدیکتراست؟
1)62)31)114)125)21""",
"""24.دنبالة{ }naبا فرمول زیر تعریف میشود:( ) ( )na n n
2 21 1
1 1 1 1
حاصل عبارتزیرچهقدر است؟a a a1 2 96
4 4 4
1)182412)18625 11)186254)19013 15)19013""",
"""25.دانشمندان سیارهایکوچک به شعاع30کیلومتر کشف کردهاند که تمام سطآ آن را آس فرا
گرفته است.شهابیکرویشکلبه این سیاره برخورد میکند و کامالً در آس فرو میرود وارتفاع آس
روی سطآ سیارهیکسانتیمترافزایش پیدامیکند.شعاع شهاس چند متر بوده است؟""",
